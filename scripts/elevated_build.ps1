...
"Solution: $root\AI-CAD.sln" | Out-File -FilePath $log -Append -Encoding utf8
$ms = Get-Command msbuild -ErrorAction SilentlyContinue
if ($ms) {
    "Found msbuild at: $($ms.Path)" | Out-File -FilePath $log -Append -Encoding utf8
    & msbuild (Join-Path $root 'AI-CAD.sln') /p:Configuration=Debug /m *>&1 | Out-File -FilePath $log -Append -Encoding utf8
} else {
    $d = Get-Command dotnet -ErrorAction SilentlyContinue
    if ($d) {
        "Found dotnet at: $($d.Path)" | Out-File -FilePath $log -Append -Encoding utf8
        dotnet build (Join-Path $root 'AI-CAD.sln') -c Debug *>&1 | Out-File -FilePath $log -Append -Encoding utf8
    } else {
