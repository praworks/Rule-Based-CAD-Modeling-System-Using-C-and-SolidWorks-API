...
namespace AICAD.UI
{
    public static class LlmTab
    {
        public static Panel CreatePanel(SettingsDialog host)
        {
            ...
            // Labels and controls
            var lblLlmMode = new Label { Text = "LLM Mode:", Dock = DockStyle.Fill, Padding = new Padding(0,12,0,0), AutoSize = false, TextAlign = ContentAlignment.MiddleLeft };
            UITheme.ApplyLabelStyle(lblLlmMode);
            host._cmbLlmMode = new ComboBox { Dock = DockStyle.Fill, DropDownStyle = ComboBoxStyle.DropDownList, Margin = new Padding(0,8,0,8) };
            ...
            // Local endpoint
            var lblLocalEndpoint = new Label { Text = "Local LLM Endpoint:", Dock = DockStyle.Fill, Padding = new Padding(0,12,0,0), AutoSize = false, TextAlign = ContentAlignment.MiddleLeft };
            UITheme.ApplyLabelStyle(lblLocalEndpoint);
            host._txtLocalEndpoint = new TextBox { Dock = DockStyle.Fill, Margin = new Padding(0,8,0,8) };
            UITheme.ApplyTextBoxStyle(host._txtLocalEndpoint);

            // Gemini key
            var lblGemini = new Label { Text = "Google Gemini API Key:", Dock = DockStyle.Fill, Padding = new Padding(0,12,0,0), AutoSize = false, TextAlign = ContentAlignment.MiddleLeft };
            UITheme.ApplyLabelStyle(lblGemini);
            host._txtGeminiKey = new TextBox { Dock = DockStyle.Fill, UseSystemPasswordChar = true, Margin = new Padding(0,8,0,8) };
            UITheme.ApplyTextBoxStyle(host._txtGeminiKey);
            var geminiPanel = new Panel { Dock = DockStyle.Fill };
            var btnToggleGemini = new Button { Width = 44, Height = 24, Dock = DockStyle.Right, FlatStyle = FlatStyle.Flat, Text = "Show", TabStop = false, Margin = new Padding(6,8,0,8) };
            ...
            // Groq key
            var lblGroq = new Label { Text = "Groq API Key:", Dock = DockStyle.Fill, Padding = new Padding(0,12,0,0), AutoSize = false, TextAlign = ContentAlignment.MiddleLeft };
            UITheme.ApplyLabelStyle(lblGroq);
            host._txtGroqKey = new TextBox { Dock = DockStyle.Fill, UseSystemPasswordChar = true, Margin = new Padding(0,8,0,8) };
            UITheme.ApplyTextBoxStyle(host._txtGroqKey);
            var groqPanel = new Panel { Dock = DockStyle.Fill };
            var btnToggleGroq = new Button { Width = 44, Height = 24, Dock = DockStyle.Right, FlatStyle = FlatStyle.Flat, Text = "Show", TabStop = false, Margin = new Padding(6,8,0,8) };
            ...
            // Local model
            var lblLocalModel = new Label { Text = "Local Model (identifier):", Dock = DockStyle.Fill, Padding = new Padding(0,12,0,0), AutoSize = false, TextAlign = ContentAlignment.MiddleLeft };
            UITheme.ApplyLabelStyle(lblLocalModel);
            host._txtLocalModel = new TextBox { Dock = DockStyle.Fill, Margin = new Padding(0,8,0,8) };
            UITheme.ApplyTextBoxStyle(host._txtLocalModel);

            // Local system prompt
            var lblLocalSys = new Label { Text = "Local System Prompt:", Dock = DockStyle.Fill, Padding = new Padding(0,12,0,0), AutoSize = false, TextAlign = ContentAlignment.MiddleLeft };
            UITheme.ApplyLabelStyle(lblLocalSys);
            host._txtLocalSystemPrompt = new TextBox { Dock = DockStyle.Fill, Margin = new Padding(0,8,0,8) };
