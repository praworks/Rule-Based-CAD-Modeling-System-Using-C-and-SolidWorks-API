...
namespace AICAD.UI
{
    public static class NameEasyTab
    {
        public static Panel CreatePanel(SettingsDialog host)
        {
            ...
            var lblPath = new Label { Text = "Database Path:", Dock = DockStyle.Fill, Padding = new Padding(0,8,0,0) };
            UITheme.ApplyLabelStyle(lblPath);
            host._txtNameEasyPath = new TextBox { Dock = DockStyle.Fill, Margin = new Padding(0,5,0,5) };
            UITheme.ApplyTextBoxStyle(host._txtNameEasyPath);
            host._btnBrowseNameEasy = new Button { Text = "Browse...", Width = 80, Height = 26 };
            ...
            host._lblNameEasyInfo = new Label { Text = "Choose where to store the NameEasy.db database. Default: add-in folder.", Dock = DockStyle.Fill, ForeColor = Color.Gray, AutoSize = false, Padding = new Padding(0,10,0,0) };
            ...
            host._btnSaveNameEasy = new Button { Text = "Save", Width = 100, Height = 30, Anchor = AnchorStyles.Right };
